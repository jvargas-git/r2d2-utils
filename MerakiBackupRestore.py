...
def print_welcome_screen():
    ...
    banner = "=" * 70
    api_status = "YES (From Environment)" if API_KEY else "NO (Using code default)"
    github_status = "READY" if GITHUB_TOKEN and GITHUB_REPO else "NO (Missing GITHUB_TOKEN or GITHUB_REPO)"
    print(f"""{banner}
                 MERAKI DISASTER RECOVERY TOOLKIT
{banner}
This automation script processes, archives, and restores entire Cisco
Meraki organization templates, global settings, and networks.

OPERATIONAL MODES:
  [B] Backup Mode  - Extracts global policy objects/groups, template maps,
                     SSIDs, switch profiles, and bound network overrides.
                     Saves to timestamped JSON payload archives.
  [R] Restore Mode - Restores configurations back to live structures.
                     Automatically calculates ID transformation matrices
                     and supports granular single-network targeting.

CLI FLAGS & PARAMETERS:
  -B, --backup     - Triggers the backup routine for templates & networks.
  -R, --restore    - Triggers the restoration engine from local backups.
  -A, --all        - Runs automatically across all matching properties.
                     Bypasses interactive prompts and this welcome screen.
  -O, --orgname    - Specifies the exact target Meraki Organization Name.
  -N, --network    - Isolates restoration strictly to a single named network
                     by parsing all files to auto-locate its backup layer.
  -G, --github     - Routes backup payloads directly to a remote GitHub
                     repository instead of local disk storage.

CONFIGURATION & PATHS:
  Target Local Directory   : {os.path.abspath(BACKUP_DIR)}
  API Key Detected         : {api_status}
  GitHub Export Enabled    : {github_status}
{banner}
Version 1.2
""")
...
def safe_get(label, func, *args, **kwargs):
    """Calls a Meraki API getter and returns its result, or None on failure (logged as a warning)."""
    try:
        return func(*args, **kwargs)
    except meraki.APIError as e:
        logging.warning(f"    [!] Could not fetch {label}: {e}")
        return None


def assign_if_present(target_dict, key, label, func, *args, **kwargs):
    """Fetches a value via safe_get and stores it in target_dict[key] only if the call succeeded."""
    result = safe_get(label, func, *args, **kwargs)
    if result is not None:
        target_dict[key] = result
    return result


def safe_do(label, func, *args, **kwargs):
    """Calls a Meraki API write operation; returns True/False, logging any failure as a warning."""
    try:
        func(*args, **kwargs)
        return True
...
def run_backup(dashboard, org_id, target_template_id=None, use_github=False, templates_cache=None):
    ...
    try:
        templates = templates_cache if templates_cache is not None else dashboard.organizations.getOrganizationConfigTemplates(org_id)
    ...
    global_policy_objects = safe_get("global Policy Objects", dashboard.organizations.getOrganizationPolicyObjects, org_id, total_pages='all') or []
    global_policy_groups = safe_get("global Policy Object Groups", dashboard.organizations.getOrganizationPolicyObjectsGroups, org_id, total_pages='all') or []
    ...
    for template in templates:
        ...
        product_types = template.get("productTypes", [])
        ...
        net_settings = template_data["network_settings"]
        assign_if_present(net_settings, "settings", "network settings", dashboard.networks.getNetworkSettings, template_id)
        assign_if_present(net_settings, "alerts", "network alerts settings", dashboard.networks.getNetworkAlertsSettings, template_id)
        assign_if_present(net_settings, "snmp", "network SNMP settings", dashboard.networks.getNetworkSnmp, template_id)
        assign_if_present(net_settings, "syslog", "network syslog servers", dashboard.networks.getNetworkSyslogServers, template_id)

        if "appliance" in product_types:
            app_settings = template_data["appliance_settings"]
            assign_if_present(app_settings, "vlans", "appliance VLANs", dashboard.appliance.getNetworkApplianceVlans, template_id)
            assign_if_present(app_settings, "static_routes", "appliance static routes", dashboard.appliance.getNetworkApplianceStaticRoutes, template_id)
            assign_if_present(app_settings, "ports", "appliance ports", dashboard.appliance.getNetworkAppliancePorts, template_id)
            assign_if_present(app_settings, "l3_firewall", "appliance L3 firewall rules", dashboard.appliance.getNetworkApplianceFirewallL3FirewallRules, template_id)
            assign_if_present(app_settings, "l7_firewall", "appliance L7 firewall rules", dashboard.appliance.getNetworkApplianceFirewallL7FirewallRules, template_id)
            assign_if_present(app_settings, "vpn_bgp_sdwan_rules", "appliance uplink selection rules", dashboard.appliance.getNetworkApplianceTrafficShapingUplinkSelection, template_id)

        if "wireless" in product_types:
            wireless_settings = template_data["wireless_settings"]
            assign_if_present(wireless_settings, "settings", "wireless settings", dashboard.wireless.getNetworkWirelessSettings, template_id)
            ssids = safe_get("wireless SSIDs", dashboard.wireless.getNetworkWirelessSsids, template_id)
            if ssids is not None:
                wireless_settings["ssids"] = ssids
                wireless_settings["ssid_details"] = {}
                for ssid in ssids:
                    num = ssid["number"]
                    ssid_detail = {}
                    assign_if_present(ssid_detail, "l3_firewall", f"SSID {num} L3 firewall rules",
                                       dashboard.wireless.getNetworkWirelessSsidFirewallL3FirewallRules, template_id, num)
                    wireless_settings["ssid_details"][num] = ssid_detail

        if "switch" in product_types:
            profiles = safe_get("switch profiles", dashboard.switch.getOrganizationConfigTemplateSwitchProfiles, org_id, template_id) or []
            for p in profiles:
                ports = safe_get(f"ports for switch profile {p['switchProfileId']}",
                                  dashboard.switch.getOrganizationConfigTemplateSwitchProfilePorts, org_id, template_id, p["switchProfileId"])
                template_data["switch_profiles"].append({"config": p, "ports": ports or []})
        ...
        for net in bound_networks:
            ...
            devices = safe_get(f"devices for network {net['name']}", dashboard.networks.getNetworkDevices, net_id)
            if devices is not None:
                local_overrides["devices"] = devices
                for device in devices:
                    if "MX" in device.get("model", "") or "Z" in device.get("model", ""):
                        uplinks = safe_get(f"uplink settings for {device['serial']}",
                                           dashboard.appliance.getDeviceApplianceUplinksSettings, device["serial"])
                        if uplinks is not None:
                            local_overrides["wan_uplink_settings"][device["serial"]] = uplinks

            assign_if_present(local_overrides, "mx_vlans", f"VLANs for network {net['name']}", dashboard.appliance.getNetworkApplianceVlans, net_id)
            assign_if_present(local_overrides, "site_to_site_vpn", f"Site-to-Site VPN for network {net['name']}", dashboard.appliance.getNetworkApplianceVpnSiteToSiteVpn, net_id)

            template_data["networks"].append({"config": net, "local_overrides": local_overrides})
...
def run_restore(dashboard, org_id, target_filename=None, auto_detect_deletion=False, target_network_name=None):
    ...
    # Global policy objects/groups are org-wide, not per-template — fetch once for the
    # whole restore run and keep the in-memory copies updated as new ones are created,
    # instead of re-querying the API for every backup file processed below.
    live_objects = safe_get("live Policy Objects", dashboard.organizations.getOrganizationPolicyObjects, org_id, total_pages='all') or []
    live_groups = safe_get("live Policy Object Groups", dashboard.organizations.getOrganizationPolicyObjectsGroups, org_id, total_pages='all') or []
    ...
    for file in files:
        ...
        if is_isolated_net_run:
            ...
            if not backup_data["networks"]:
                ...
                continue
            was_deleted = not any(n["name"].lower() == target_network_name.lower() for n in org_networks)
        elif auto_detect_deletion:
            was_deleted = not any(t["name"] == template_name for t in org_templates)
        else:
            was_deleted_str = input(f"Is this a full template/org disaster rebuild for {template_name}? (yes/no): ").strip().lower()
            was_deleted = was_deleted_str in ('y', 'yes')
        ...
        for obj in backup_data.get("global_policy_objects", []):
            try:
                ...
                logging.warning(f"    [!] Could not sync policy object '{obj.get('name')}': {e}")
        ...
        for group in backup_data.get("global_policy_object_groups", []):
            ...
            if not match_group:
                try:
                    new_group = dashboard.organizations.createOrganizationPolicyObjectsGroup(org_id, name=group['name'], objectIds=mapped_ids)
                    live_groups.append(new_group)
                except Exception as e:
                    logging.warning(f"    [!] Could not create policy object group '{group['name']}': {e}")
            else:
                safe_do(f"Update policy object group '{group['name']}'",
                        dashboard.organizations.updateOrganizationPolicyObjectsGroup, match_group['id'], name=group['name'], objectIds=mapped_ids)
        ...
        if was_deleted and not is_isolated_net_run:
            ...
            org_templates.append(new_template)
        else:
        ...
        # --- RESTORE GLOBAL BASELINE AND APPLIANCE LAYERS ONLY DURING FULL RUNS ---
        if not is_isolated_net_run:
            ...
            if "settings" in net_settings:
                clean_settings = {k: v for k, v in net_settings["settings"].items() if v is not None}
                safe_do("Update network settings", dashboard.networks.updateNetworkSettings, target_template_id, **clean_settings)

            app_settings = backup_data.get("appliance_settings", {})
            if "appliance" in product_types and app_settings:
                # Instantiate VLAN subnets before committing routes that depend on them.
                if "vlans" in app_settings:
                    logging.info("     -> Establishing Core Template Layer-3 VLAN boundaries...")
                    for vlan in app_settings["vlans"]:
                        safe_do(f"Create VLAN {vlan['id']}", dashboard.appliance.createNetworkApplianceVlan,
                                target_template_id, id=vlan["id"], name=vlan["name"], subnet=vlan["subnet"], applianceIp=vlan["applianceIp"])
                ...
                if "static_routes" in app_settings:
                    ...
                    for route in app_settings["static_routes"]:
                        clean_route = {k: v for k, v in route.items() if k != 'id'}
                        safe_do("Create static route", dashboard.appliance.createNetworkApplianceStaticRoute, target_template_id, **clean_route)

                if "l3_firewall" in app_settings:
                    safe_do("Update L3 firewall rules", dashboard.appliance.updateNetworkApplianceFirewallL3FirewallRules,
                            target_template_id, **app_settings["l3_firewall"])

        # Track networks needing VPN activation after they are initialized and fully ready
        vpn_rebuild_queue = []

        # --- BOUND NETWORKS RE-INJECTION ---
        for net_obj in backup_data.get("networks", []):
            ...
            if not target_net:
                ...
                safe_do(f"Bind network {old_net_config['name']} to template", dashboard.networks.bindNetwork, current_net_id, configTemplateId=target_template_id)
            ...
            for serial, uplinks in overrides.get("wan_uplink_settings", {}).items():
                interfaces = uplinks.get("interfaces", {})
                safe_do(f"Update uplink settings for {serial}", dashboard.appliance.updateDeviceApplianceUplinksSettings,
                        serial, wan1=interfaces.get("wan1", {}), wan2=interfaces.get("wan2", {}))

            # Local VLAN overrides must land before any routing rules that depend on them.
            for vlan in overrides.get("mx_vlans", []):
                safe_do(f"Create local VLAN {vlan['id']}", dashboard.appliance.createNetworkApplianceVlan,
                        current_net_id, id=vlan["id"], name=vlan["name"], subnet=vlan.get("subnet"), applianceIp=vlan.get("applianceIp"))

            # Queued rather than applied immediately, since VPN mesh setup is order-sensitive
            # across networks and should run only after every network shell is ready.
        ...
        if vpn_rebuild_queue:
            ...
            for vpn_job in vpn_rebuild_queue:
                ...
                safe_do(f"VPN mesh setup for network {vpn_job['name']}", dashboard.appliance.updateNetworkApplianceVpnSiteToSiteVpn,
                        vpn_job["net_id"], **clean_payload)
    ...
    if args.backup:
        if args.all:
            ...
            try:
                ...
                run_backup(dashboard_session, org_id, target_template_id=templates[sel]['id'], use_github=args.github, templates_cache=templates)
